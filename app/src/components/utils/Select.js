...
const BaseSelect = (props, children) => {
  ...
  return Row({ class: containerBase }, [
    ...
    IonIcon({
      ...
      size: 'small',
      class:
        'pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-400 sm:size-4',
    }),
  ]);
};
...
const SelectRelative = (props, children) => {
  ...
  const outerClass = `rounded-md bg-white/5 pl-3 outline-1 -outline-offset-1 outline-gray-600
   has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-indigo-500`

  const baseClass = `${outerClass} col-start-1 row-start-1 w-full appearance-none rounded-md bg-gray-800 
    py-1.5 pr-7 pl-3 text-small text-gray-400 *:bg-gray-800 
