...
export function HoldOrders(props) {
  ...
  const handleSearchChange = (e) => {
    ...
    const timeout = setTimeout(() => {
      props.viewModel.updateHoldOrderTableConfig({ search: value, offset: 0 });
    }, 500);
  ...
  const handleFilterClick = (filterKey) => {
    props.viewModel.updateHoldOrderTableConfig({ filter: filterKey, offset: 0 });
  };
  ...
  const handleSetLimit = (newLimit) => {
    props.viewModel.updateHoldOrderTableConfig({ limit: newLimit, offset: 0 });
  };
  const handlePreviousPage = () => {
    props.viewModel.updateHoldOrderTableConfig({ offset: Math.max(0, paginationOffset - paginationLimit) });
  };
  const handleNextPage = () => {
    props.viewModel.updateHoldOrderTableConfig({ offset: paginationOffset + paginationLimit });
  };
  ...
  const handleViewHoldOrder = (holdOrderId) => {
    props.viewModel.openHoldOrderDrawer(holdOrderId);
  };
  ...
  const handleDeleteHoldOrder = async (holdOrderId) => {
    try {
      const confirmed = await showConfirmation({
        ...
        message: 'Are you sure you want to delete this hold order?',
      });
      if (!confirmed) return;
  ...
  return Row({ class: 'flex-1 flex flex-col min-h-0 overflow-hidden' }, [
    // Filter row (small buttons) – above search row
    Row({ class: 'flex items-center gap-2 px-4 py-2 border-b border-gray-200 bg-gray-50 flex-shrink-0 flex-wrap' }, [
      ...
      Button({
        variant: currentFilter === 'archived' ? 'primary' : 'outline',
        class: FILTER_BTN_CLASS,
      ...
      loading
        ...
        : holdOrders.length === 0
          ? Row({ class: 'flex flex-col items-center justify-center gap-2 p-8 text-center text-gray-500 border border-gray-200 rounded-lg bg-gray-50' }, [
              ...
              Row({ class: 'text-xs text-gray-400 max-w-sm' }, 'Hold orders are drafts saved from the Current Order tab. Use Checkout → Hold to save one.'),
            ])
              ...
              TableBody({}, [
                holdOrders.map((holdOrder, index) =>
                  TableRow({ key: holdOrder.id }, [
                    ...
                    TableDCell({}, formatDateDDMMYYYY(holdOrder.order_date)),
    ...
    // Hold Order Details Drawer (pattern: Stock/Products – drawer state in VM, render when selected + open)
    selectedHoldOrder && HoldOrderDetailsDrawer({
      holdOrder: selectedHoldOrder,
      showSlide: holdDrawerOpen,
      onClose: () => props.viewModel.closeHoldOrderDrawer(),
      ...props
    }),
    ]),
  ])
...
// Hold Order Details Drawer – read-only (same pattern as Stock ViewDetailsDrawer / ProductDetails)
  ...
  return Drawer({ class: 'h-full overflow-y-auto flex flex-col', openSlide: showSlide }, [
    CardHeader({ class: 'px-6 flex items-center justify-between h-12 border-b border-gray-200' }, [
      Row({ class: 'flex items-center gap-3' }, [
        IonIcon({ name: 'document-text-outline', class: 'text-xl text-indigo-600' }),
        Row({ class: 'text-lg font-semibold text-gray-800' }, 'Hold Order Details'),
      ]),
      ...
      Row({ class: 'flex flex-col gap-5' }, [
        Row({ class: 'grid grid-cols-2 gap-x-6 gap-y-3' }, [
          Row({}, [
            ...
            Row({ class: 'text-sm text-gray-900' }, holdOrder.supplier_name || 'Unknown'),
          ]),
          Row({}, [
            Row({ class: 'text-xs font-medium uppercase tracking-wide text-gray-500 mb-0.5' }, 'Order Date'),
            Row({ class: 'text-sm text-gray-900' }, formatDateDDMMYYYY(holdOrder.order_date)),
          ]),
          Row({}, [
            Row({ class: 'text-xs font-medium uppercase tracking-wide text-gray-500 mb-0.5' }, 'Payment Mode'),
            Row({ class: 'text-sm text-gray-900 capitalize' }, holdOrder.payment_mode || 'cash'),
          ]),
        ]),
    ...
    CardFooter({ class: 'flex justify-end gap-2 px-6 py-3 border-t border-gray-200 flex-shrink-0' }, [
