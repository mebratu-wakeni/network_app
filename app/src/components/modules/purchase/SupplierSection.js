...
function SearchSupplier(props) {
  ...
  const supplierDropdownLoading = props.viewModel.getState('supplier-dropdown-loading') === true;
  ...
  const suppliers = props.viewModel.getState('supplier-list') || [];

  const displayClosed = selectedSupplier
    ? (selectedSupplier.name || '')
    : supplierSearchQuery || 'Search suppliers…';
  ...
  const menuRows = [];
  if (supplierDropdownLoading) {
    menuRows.push(Row({ key: 'sup-dd-loading', class: 'px-3 py-2 text-xs text-gray-500 italic' }, 'Searching…'));
  } else if (suppliers.length === 0) {
    menuRows.push(
      Row(
        { key: 'sup-dd-empty', class: 'px-3 py-2 text-xs text-gray-500' },
        supplierSearchQuery.trim() ? 'No suppliers match your search.' : 'No suppliers found. Add them in Customer Management.'
      )
    );
  } else {
    menuRows.push(
        ...
        const partnerChildren = [
          ...
          Row({ class: 'flex items-center gap-2 text-xs text-gray-500' }, [
            ...(supplier.contact_person
              ? [Row({ class: 'flex items-center gap-1' }, [IonIcon({ name: 'person-outline', class: 'text-xs' }), supplier.contact_person])]
              : []),
  ...
  return DropdownSearch({
    ...
    value: showSupplierDropdown ? supplierSearchQuery : displayClosed,
    placeholder: 'Search suppliers...',
    onInput: handleSupplierSearch,
    onFocus: () => {
      props.viewModel.loadSuppliers(props.viewModel.getState('supplier-search-query') || '');
