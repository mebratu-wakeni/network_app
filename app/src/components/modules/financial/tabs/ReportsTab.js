...
function BalanceSheetView({ data }) {
  const { as_of_date, assets, liabilities, equity, total_liabilities_equity, reconciliation, unmapped_ledger_accounts, coa_hierarchy_conflicts } =
    data || {}
  ...
  const ledgerSum = reconciliation?.ledger_sum_check
  const reconHints = reconciliation?.hints || []
  const eqBreak = reconciliation?.equation_breakdown
  const logicTag = reconciliation?.balance_sheet_logic
  const unmapped = unmapped_ledger_accounts || []
  const coaConflicts = coa_hierarchy_conflicts || []
  ...
  return Row({ class: 'max-w-2xl' }, [
    ...
    reconciliation &&
      Row(
        {
          class: `mb-4 rounded-lg border px-3 py-2 text-sm ${balanced ? 'border-green-200 bg-green-50 text-green-900' : 'border-amber-300 bg-amber-50 text-amber-950'}`,
        },
        [
          Row({ class: 'font-semibold' }, balanced ? 'Balance sheet equation: OK' : 'Balance sheet does not tie — review details below'),
          !balanced &&
            variance != null &&
            Row({ class: 'mt-1' }, [
              `Assets minus Liabilities & Equity = Br ${financeFormat(variance)} (should be 0.00). `,
              'Often this is ledger activity on a missing/inactive code, a type outside A/L/E/R/E, a level-3+ account, or balances posted only to a parent header (e.g. 1000) while children are empty — check “accounts not reflected” and COA parent/child warnings below.',
            ]),
          ledgerSum != null &&
            Math.abs(ledgerSum) > 0.02 &&
            Row({ class: 'mt-1 text-xs opacity-90' }, [
              `Σ all ledger balances to date: Br ${financeFormat(ledgerSum)} — in double-entry books this should be 0; if not, check posting integrity.`,
            ]),
          !balanced &&
            Row({ class: 'mt-2 text-xs text-amber-900/90 border-t border-amber-200/80 pt-2' }, [
              'Reports are generated by the API (default http://localhost:4000/api). After code changes, restart that Node process or reinstall the desktop app so the bundled API is refreshed — otherwise numbers stay on an old server.',
            ]),
          reconHints.length > 0 &&
            Row({ class: 'mt-2 space-y-1' }, [
              Row({ class: 'text-xs font-semibold' }, 'What to check next'),
              ...reconHints.map((h, i) => Row({ key: i, class: 'text-xs leading-snug' }, `• ${h}`)),
            ]),
          !balanced &&
            eqBreak &&
            Row({ class: 'mt-3 rounded border border-amber-200 bg-white/60 px-2 py-2 text-xs' }, [
              Row({ class: 'font-semibold mb-1' }, 'Equation breakdown'),
              Row({ class: lineClass }, [
                Row({ class: 'text-gray-700' }, 'Total assets'),
                Row({ class: 'tabular-nums' }, `Br ${financeFormat(eqBreak.total_assets)}`),
              ]),
              Row({ class: lineClass }, [
                Row({ class: 'text-gray-700' }, 'Total liabilities'),
                Row({ class: 'tabular-nums' }, `Br ${financeFormat(eqBreak.total_liabilities)}`),
              ]),
              Row({ class: lineClass }, [
                Row({ class: 'text-gray-700' }, 'Equity (GL accounts only, before NI line)'),
                Row({ class: 'tabular-nums' }, `Br ${financeFormat(eqBreak.equity_accounts_before_ni)}`),
              ]),
              Row({ class: lineClass }, [
                Row({ class: 'text-gray-700' }, 'Net income bridged from Revenue − Expense'),
                Row({ class: 'tabular-nums' }, `Br ${financeFormat(eqBreak.net_income_bridged)}`),
              ]),
              Row({ class: 'flex justify-between pt-1 font-semibold border-t border-amber-200' }, [
                Row({ class: 'text-gray-900' }, 'Total liabilities + equity'),
                Row({ class: 'tabular-nums' }, `Br ${financeFormat(eqBreak.total_liabilities_and_equity)}`),
              ]),
              logicTag &&
                Row({ class: 'mt-1 text-[10px] text-gray-500' }, [`Statement logic: ${logicTag}`]),
            ]),
        ].filter(Boolean)
      ),
    unmapped.length > 0 &&
      Row({ class: 'mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-900' }, [
        Row({ class: 'font-semibold' }, 'Accounts not fully reflected on this statement'),
        ...unmapped.map((u) =>
          Row({ key: u.account_code, class: 'mt-1 text-xs' }, [
            `${u.account_code} ${u.account_name}: balance Br ${financeFormat(u.balance)} — ${u.note || ''}`,
          ])
        ),
      ]),
    coaConflicts.length > 0 &&
      Row({ class: 'mb-4 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-950' }, [
        Row({ class: 'font-semibold' }, 'Chart of accounts: parent and child both have balances'),
        ...coaConflicts.map((c) =>
          Row({ key: c.account_code, class: 'mt-1 text-xs' }, [
            `${c.account_code} ${c.account_name} (${c.account_type}): Br ${financeFormat(c.balance)} — ${c.note || ''}`,
          ])
        ),
      ]),
    ...
    !balanced &&
      totalAssets != null &&
      total_liabilities_equity != null &&
      Row({ class: 'mt-2 text-sm font-medium text-amber-800' }, [
        `Cross-check: Total Assets Br ${financeFormat(totalAssets)} vs Liabilities & Equity Br ${financeFormat(total_liabilities_equity)}`,
      ]),
  ].filter(Boolean))
}
